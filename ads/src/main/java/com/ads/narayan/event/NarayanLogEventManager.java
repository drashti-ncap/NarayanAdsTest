...
public class NarayanLogEventManager {
    ...
    public static void logPaidAdImpression(Context context, AdValue adValue, String adUnitId, String mediationAdapterClassName, AdType adType) {
        logEventWithAds(context, (float) adValue.getValueMicros(), adValue.getPrecisionType(), adUnitId, mediationAdapterClassName, NarayanAdConfig.PROVIDER_ADMOB);
        NarayanAdjust.pushTrackEventAdmob(adValue);
        NarayanAppsflyer.getInstance().pushTrackEventAdmob(adValue, adUnitId, adType);
    }

    public static void logPaidAdImpression(Context context, MaxAd adValue, AdType adType) {
        logEventWithAds(context, (float) adValue.getRevenue(), 0, adValue.getAdUnitId(), adValue.getNetworkName(), NarayanAdConfig.PROVIDER_MAX);
        NarayanAdjust.pushTrackEventApplovin(adValue, context);
        NarayanAppsflyer.getInstance().pushTrackEventApplovin(adValue, adType);
    }
    ...
    private static void logPaidAdImpressionValue(Context context, double value, int precision, String adunitid, String network, int mediationProvider) {
        ...
        NarayanAdjust.logPaidAdImpressionValue(value, "USD");
    ...
    public static void setEventNamePurchaseAdjust(String eventNamePurchase) {
        NarayanAdjust.setEventNamePurchase(eventNamePurchase);
    }

    public static void trackAdRevenue(String id) {
        NarayanAdjust.trackAdRevenue(id);
    }

    public static void onTrackEvent(String eventName) {
        NarayanAdjust.onTrackEvent(eventName);
    }

    public static void onTrackEvent(String eventName, String id) {
        NarayanAdjust.onTrackEvent(eventName, id);
    }

    public static void onTrackRevenue(String eventName, float revenue, String currency) {
        NarayanAdjust.onTrackRevenue(eventName, revenue, currency);
    }

    public static void onTrackRevenuePurchase(float revenue, String currency, String idPurchase, int typeIAP) {
        NarayanAdjust.onTrackRevenuePurchase(revenue, currency);
        NarayanAppsflyer.getInstance().onTrackRevenuePurchase(revenue, currency, idPurchase, typeIAP);
    }
    ...
    public static void pushTrackEventApplovin(MaxAd ad, Context context) {
        NarayanAdjust.pushTrackEventApplovin(ad, context);
    }
}
